...
class crm extends CI_Controller {
    ...
    function __construct() {
        ...
        $delerInfo = $this->crm_dealer->getUserDealerId($userId);
        if (!$this->ion_auth->logged_in() && !$this->ion_auth->in_group('dealer')) {
            redirect($base);
            //echo 'NIET INGELOGD';
        } else {
            // $this->dealerId = 00010;//'00010';//$this->session->userdata['imobycode'];
            $userInfo = $this->ion_auth->user()->row();
            $this->klantnummer = $userInfo->klantnummer;
            //'00010';//$this->session->userdata['imobycode'];           
            //$this->dealerId = 97940;//'00010';
            //$this->session->userdata['imobycode'];           
            $this->dealerId = $delerInfo[0]->dealer; //'00010';//$this->session->userdata['imobycode'];           
            $this->localDealerDir = $_SERVER['DOCUMENT_ROOT']. '/fileserver/bofiles/crm_files/' . $this->dealerId . '/';
            ...
            $this->crmDealerUrl = base_url().'/fileserver/bofiles/crm_files/' . $this->dealerId . '/';
            $this->remoteDealerDir = $_SERVER['DOCUMENT_ROOT'].'/fileserver/bofiles/downloads/' . $this->dealerId . '/'; // get images from this
            $this->remoteDealerPath = $_SERVER['DOCUMENT_ROOT'].'/fileserver/bofiles/downloads/' . $this->dealerId . '/';
            $this->load->library('crm_breadcrumbs');
            $this->crm_breadcrumbs->setCrumb(array('step' => 0, 'link' => '/backoffice/relatiebeheer/relaties', 'title' => 'Relatiebeheer'));
        }
        if ($this->errorMessage) {

            $this->session->errorMessage = $this->errorMessage;
        }
      //echo $this->localDealerDir;
      //echo '<br>';
      //echo $this->autoPath;
      //die();
...
// OVERVIEW FUNCTIONS       
    function index() {
        ...
        if ($this->_checkDealerInfo($dealer) === false) {
            ...
            redirect(base_url() . 'backoffice/relatiebeheer/settings');
        } else {

            redirect(base_url() . 'backoffice/relatiebeheer/relaties');
        }
    }
...
//## KLANT FUNCTIONS    
    ...
    function relaties() {
        ...
        $this->crm_breadcrumbs->setCrumb(array('step' => 1, 'link' => 'backoffice/relatiebeheer/relaties', 'title' => 'Relaties'), true);
        ...
        if (count($autos)) {
            foreach ($autos as $key => $autoData) {
                ...
                $autos[$key]->klant_naam = $autoData->klant_voornaam . (($autoData->klant_tussenvoegsel) ? ' ' . $autoData->klant_tussenvoegsel : '') . ' ' . $autoData->klant_achternaam;
...
// AUTO FUNCTIONS
    function auto($autoId=false, $mode=false, $extra=false) {
        ...
        if ($mode) {
            ...
            switch ($mode) {
                ...
                case 'marketingSetting':
                    ...
                    break;
                case 'activeSetting':
                    if (is_numeric($this->input->post('auto_actief', true))) {
                        $this->crm_autos->setActive($autoId, $this->input->post('auto_actief', true));
                    }
                    exit();
                    break;
                case 'upload':
        ...
        if ($autoId <= 0) {
            redirect('relatiebeheer/relaties');
            exit();
        }
        ...
        $this->crm_breadcrumbs->setCrumb(array('step' => 1, 'link' => '/relatiebeheer/relaties', 'title' => 'Relaties'));
        $this->crm_breadcrumbs->setCrumb(array('step' => 2, 'link' => '/relatiebeheer/relatie/' . $auto->klant_id, 'title' => $auto->klant_voornaam[0] . '. ' . (($auto->klant_tussenvoegsel) ? $auto->klant_tussenvoegsel . ' ' : '') . ' ' . $auto->klant_achternaam));
        $this->crm_breadcrumbs->setCrumb(array('step' => 3, 'link' => '/relatiebeheer/auto/' . $auto->auto_id, 'title' => $auto->auto_merk . ' ' . $auto->auto_type));
...
//### format saving data BEFORE RDW SESSION DATA        
        ...
        // save picture
        if (isset($saveAutoData['auto_fotos'])) {
            
         
            $fileLocation = false;
            if (strpos('http://', $saveAutoData['auto_fotos']) === false) {
                //$fileLocation = $this->remoteDealerPath . $saveAutoData['auto_fotos'];
                $fileLocation = $saveAutoData['auto_fotos'];
            } else {
                $fileLocation = $saveAutoData['auto_fotos'];
            }
    ...
    function ongekoppeld($kenteken=false) {
        ...
        if (count($autos)) {
            ...
            foreach ($autos as $key => $autoData) {
                ...
                $autos[$key]->auto_kmstand = $autoData->KMvalue;
                $autos[$key]->auto_bouwjaar = $autoData->buildYear;
                $autos[$key]->auto_kenteken = $this->_formatKenteken($autos[$key]->licenseNumber);
                $autos[$key]->auto_merk_type = $autoData->brand . ' ' . $autoData->model . ' ' . $autoData->type;

                $remote = $autoData->remoteFile;
                $carimg = explode(',', $remote);
                $autos[$key]->foto = $carimg[0];
                $autos[$key]->auto_archivedate = $autoData->dateEnd;
...
// MARKETING FUNCTIONS
    ...
    private function _marketingSetting($type, $typeId) {
        ...
        if (!is_numeric($marketingId)) {
            die('Maketing niet opgeslagen.');
        }
        $marketingActief = preg_replace("/[^0-9]/", "", $this->input->post('marketingWaarde'));
        if (!is_numeric($marketingActief)) {
            die('Maketing niet opgeslagen');
        }
                    ...
                    if (($autoData) || ($autoDataIMOBY)) {
                        ...
                        if ($autoDataIMOBY) {
                          ...
                          //      $autoData->inrichting = $autoDataIMOBY->carrosserievorm;
                                ...
                                $autoData->vervaldatumapk = $autoDataIMOBY->APKdate;
                                $autoData->datumeersteafgiftenederland = '';
                                $autoData->datumaanvangtenaamstelling = '';
                            }

                            // Add imoby data
                            $autoData->kilometerstand = $autoDataIMOBY->KMvalue;
                            $autoData->transmissie = $autoDataIMOBY->transmissie;
                            $autoData->bouwjaar = $autoDataIMOBY->bouwjaar;
                            $autoData->prijs = $autoDataIMOBY->showroompris;
                            $remote = $autoDataIMOBY->remoteFile;
                            $carimg = explode(',', $remote);
                            $autoData->fotos = $carimg[0]; 
...
/*
   ...
   // public function getRdwData($kenteken) {
        $format = 'json';
        // new format data stat 
        $ServiceRootURL = 'https://api.datamarket.azure.com/opendata.rdw/VRTG.Open.Data/v1/';
        $request = urlencode('\'' . str_replace("-", "",$kenteken) . '\'');
        $WebSearchURL = $ServiceRootURL . 'KENT_VRTG_O_DAT(' . $request . ')?$format=json';
        $response = @file_get_contents($WebSearchURL);
        ...
        if (ctype_alnum($kenteken)) {
            ...
            /*
              ...
              // die(); 
            $ServiceRootURL = 'https://api.datamarket.azure.com/opendata.rdw/VRTG.Open.Data/v1/';
           echo  $request = urlencode('\'' . str_replace("-", "",$kenteken) . '\'');
            $WebSearchURL = $ServiceRootURL . 'KENT_VRTG_O_DAT(' . $request . ')?$format=json';
            $response = @file_get_contents($WebSearchURL);
...
// LAYOUT FUNCTIONS
    ...
    function _buildMenu($active) {
        ...
        // get items from db
        $menuItems = array();
        $menuItems['Relaties'] = array('link' => base_url() . 'backoffice/relatiebeheer/relaties', 'active' => false);
        $menuItems['Facturen'] = array('link' => base_url() . 'backoffice/relatiebeheer/facturen', 'active' => false);
        $menuItems['Marketing'] = array('link' => base_url() . 'backoffice/relatiebeheer/marketing', 'active' => false);
        $menuItems["Ongekoppelde auto's"] = array('link' => base_url() . 'backoffice/relatiebeheer/ongekoppeld', 'active' => false);
    ...
    function _buildMenusetting($active) {

        // get items from db
        $menuItems = array();
        $menuItems['Profiel'] = array('link' => base_url() . 'backoffice/instellingen/profiel', 'active' => false);
        $menuItems['Huisstijl'] = array('link' => base_url() . 'backoffice/instellingen/huisstijl', 'active' => false);
        $menuItems['Social Media'] = array('link' => base_url() . 'backoffice/instellingen/socialmedia', 'active' => false);
        $menuItems["Relatiebeheer"] = array('link' => base_url() . 'backoffice/relatiebeheer/settings', 'active' => true);
        //$menuItems['Something'] = array('link' => BASE.'crm/something', 'active' => false);
        if ($menuItems[$active]) {
            $menuItems[$active]['active'] = true;
        }
    ...
    function _buildPage($view, $data) {
        ...
        if ($data['leftMenu']['menuActive'] == 'Setting') {
            $data['leftMenu']['menuItems'] = $this->_buildMenusetting($data['leftMenu']['menuActive']);
        } else {
        ...
        $imobyMobileCode = $this->klantnummer; /* need to change at last */
        ...
        // $data['headerData']['active'] = 'Relaties';
        $data['headerData']['active'] = $data['leftMenu']['menuActive'];
